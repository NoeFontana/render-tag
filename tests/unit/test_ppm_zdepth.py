...
def test_ppm_zdepth_consistency():
    ...
    assert np.isclose(ppm_center, ppm_edge), f"PPM center ({ppm_center}) != PPM edge ({ppm_edge}) at same Z-depth"
...
def test_compute_geometric_metadata_zdepth(
    ...
    mock_bridge.bproc.camera.get_intrinsics_as_K_matrix.return_value = [[500, 0, 0], [0, 500, 0], [0, 0, 1]]
    ...
    args, kwargs = mock_ppm.call_args
