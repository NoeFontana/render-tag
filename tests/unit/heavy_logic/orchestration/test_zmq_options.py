...
def test_send_command_sets_send_timeout():
    ...
    with patch("zmq.Context") as mock_ctx_cls:
        ...
        valid_response = Response(
            status=ResponseStatus.SUCCESS, request_id="test", message="ok"
        ).model_dump_json()
        ...
        calls = [call(zmq.RCVTIMEO, short_timeout), call(zmq.SNDTIMEO, short_timeout)]
        ...
        reset_calls = [call(zmq.RCVTIMEO, 300000), call(zmq.SNDTIMEO, 300000)]
...
def test_send_command_defaults():
    ...
    with patch("zmq.Context") as mock_ctx_cls:
        ...
        calls = [call(zmq.RCVTIMEO, default_timeout), call(zmq.SNDTIMEO, default_timeout)]
        mock_socket.setsockopt.assert_has_calls(calls, any_order=True)
