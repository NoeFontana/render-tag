...
from render_tag.core.schema.base import NoiseType, SensorNoiseConfig
...
def test_sensor_noise_config_validation():
    ...
    # Test Gaussian noise
    gaussian_config = SensorNoiseConfig(model=NoiseType.GAUSSIAN, mean=0.0, stddev=0.05)
    assert gaussian_config.model == NoiseType.GAUSSIAN
    assert gaussian_config.stddev == 0.05

    # Test Poisson noise
    poisson_config = SensorNoiseConfig(model=NoiseType.POISSON)
    assert poisson_config.model == NoiseType.POISSON

    # Test Salt and Pepper
    sp_config = SensorNoiseConfig(model=NoiseType.SALT_AND_PEPPER, salt_vs_pepper=0.5, amount=0.02)
