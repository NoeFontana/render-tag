...
def test_orchestrator_uses_monitor():
    ...
    monitor.get_snapshot.return_value = WorkerSnapshot(
        worker_id="worker-0", telemetry=telemetry, last_seen=100.0, liveness="HEALTHY"
    )

    # Call _check_worker_health
    _should_restart, _limit_exceeded = orchestrator._check_worker_health(worker, False)
...
def test_orchestrator_restarts_unresponsive():
    ...
    monitor.get_snapshot.return_value = WorkerSnapshot(
        worker_id="worker-stalled", telemetry=telemetry, last_seen=100.0, liveness="UNRESPONSIVE"
    )
