import numpy as np
import pytest
from render_tag.data_io.annotations import matrix_to_quaternion_wxyz

def test_matrix_to_quaternion_identity():
    """Verify that identity matrix converts to identity quaternion [1, 0, 0, 0]."""
    matrix = np.eye(4)
    q = matrix_to_quaternion_wxyz(matrix)
...
def test_matrix_to_quaternion_90_x():
    ...
    q = matrix_to_quaternion_wxyz(matrix)
    # expected q = [cos(45), sin(45), 0, 0] = [0.707, 0.707, 0, 0]
    expected = [np.sqrt(2)/2, np.sqrt(2)/2, 0, 0]
