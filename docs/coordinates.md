...
To ensure zero-ambiguity in pose estimation and keypoint association, every asset in `render-tag` adheres to a strict index-based orientation contract:

- **Index 0:** The physical **Top-Left (TL)** corner.
- **Winding Order:** **Strictly Clockwise (CW)** from index 0 in the object's local plane.
- **Indices:** 0=TL, 1=TR, 2=BR, 3=BL.

This order is preserved through the entire projection pipeline and is reflected in the `corners` and `keypoints` fields of all exported JSON/CSV files.
...
*   **Rotation (`rotation_quaternion`):** 
    ...
    *   *Note: Internally, Blender uses `[w, x, y, z]`, but we perform a flip at the IO boundary.*
