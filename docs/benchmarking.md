...
|--------------|-------------|-------|
| **Locus Bench Phase 1** | `configs/benchmarks/historical/locus_bench_p1.yaml` | Calibration accuracy and ground truth consistency. |
...
```bash
uv run render-tag experiment run --config configs/benchmarks/historical/locus_bench_p1.yaml --renderer-mode workbench
```
