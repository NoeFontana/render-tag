...
Objective: Build a robust, offline 3D synthetic data generation pipeline for AprilTag detection using blenderproc. Philosophy: Separation of Concerns. The CLI manages the "Job"; the Script manages the "Physics"; the Math manages the "Truth".
...
camera: resolution (w, h), fov (float), samples_per_scene (int).
