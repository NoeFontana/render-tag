    ...
    def _check_assets(self):
        ...
        for obj in self.recipe.objects:
            if obj.type == "TAG":
                ...
                if base_path:
                    # heuristic check
                    # We can't easily check exact file without re-implementing get_tag_texture_path logic
                    # relying on basic existence of directory
                    path = Path(base_path)
                    if not path.exists() and not path.parent.exists(): # Allow for relative paths from repo root logic
    ...
    def _check_geometry(self):
        ...
        for i in range(len(tag_boxes)):
            for j in range(i + 1, len(tag_boxes)):
                idx1, obj1, x1, y1, r1 = tag_boxes[i]
                idx2, obj2, x2, y2, r2 = tag_boxes[j]
                ...
                if dist < min_dist * 0.9: # 10% overlap tolerance
                    ...
                    if abs(z1 - z2) < 0.001:
                       self.warnings.append(f"Potential overlap between {obj1.name} and {obj2.name} (dist={dist:.3f}, req={min_dist:.3f})")
