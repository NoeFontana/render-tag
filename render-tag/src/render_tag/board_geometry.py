...
def validate_charuco_white_border(layout: BoardLayout) -> tuple[bool, str]:
    ...
    for sq in layout.squares:
        ...
        if sq.is_white != expected_white:
            return False, f"Square ({sq.row},{sq.col}) has wrong color for checkerboard pattern"
...
def validate_charuco_tag_filling(layout: BoardLayout) -> tuple[bool, str]:
    ...
    if len(white_squares) != len(tagged_squares):
        return False, f"White squares ({len(white_squares)}) != tagged squares ({len(tagged_squares)})"
    ...
    for i, p1 in enumerate(positions):
        for j, p2 in enumerate(positions):
            ...
            if dist < min_distance:
                return False, f"Tags at positions {i} and {j} overlap (distance={dist:.4f})"
...
def validate_board_is_centered(
    ...
    if dx > tolerance or dy > tolerance:
        return False, f"Board center ({actual_cx:.4f}, {actual_cy:.4f}) != expected ({layout.center.x}, {layout.center.y})"
...
def validate_board_dimensions(
    ...
    if abs(actual_height - expected_height) > tolerance:
        return False, f"Board height {actual_height:.4f} != expected {expected_height:.4f}"
...
def validate_marker_fits_in_square(spec: BoardSpec) -> tuple[bool, str]:
    ...
    if spec.marker_size >= spec.square_size:
        return False, f"Marker size ({spec.marker_size}) >= square size ({spec.square_size})"
