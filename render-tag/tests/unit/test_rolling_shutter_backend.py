...
with patch("render_tag.backend.camera.bpy", create=True) as mock_bpy:
    with patch("render_tag.backend.camera.bproc", create=True) as mock_bproc:
...
def test_setup_sensor_dynamics_rolling_shutter():
    ...
    with patch("render_tag.backend.camera.bpy", mock_bpy):
        with patch("render_tag.backend.camera.bproc", mock_bproc):
            with patch("render_tag.backend.camera.mathutils", create=True):
...
def test_setup_sensor_dynamics_eevee_warning():
    ...
    with patch("render_tag.backend.camera.bpy", mock_bpy):
        with patch("render_tag.backend.camera.logger") as mock_logger:
            with patch("render_tag.backend.camera.mathutils", create=True):
                setup_sensor_dynamics(pose_matrix, dynamics_recipe)
