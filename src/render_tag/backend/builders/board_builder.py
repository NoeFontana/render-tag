    ...
    def build(self, recipe: ObjectRecipe) -> list[Any]:
        ...
        if texture_path and board_cfg:
            ...
            board_obj = create_board(
                props.get("cols", 3),
                props.get("rows", 3),
                props.get("marker_size", 0.08),
                props.get("tag_family", "tag36h11"),
                props.get("square_size"),
                material_config=recipe.material,
            )
        ...
        # Common setup
        board_obj.set_location(recipe.location)
        if recipe.rotation_euler:
            board_obj.set_rotation_euler(recipe.rotation_euler)
            
        if recipe.forward_axis:
            board_obj.blender_obj["forward_axis"] = recipe.forward_axis
